...
public class HeuristicWeightsLearning {
	...
	private static final String FILENAME = "GA_blockadefeature_mutate20_co4.xml";
	...
	private static void doLearning(int popSize, int numEvo, boolean a_doMonitor) throws Exception {
		...
		conf.addGeneticOperator(new UniformCrossoverOperator(conf, 4));
		...
		try {
			...
			writer.printf("\tW4 is %f%n", (double) bestSolutionSoFar.getGene(4).getAllele());
			writer.close();
		} catch (IOException e) {
			// do something
		}
